...
+ (NSDictionary *)valueTransformersForModelClass:(Class)modelClass {
	...
	for (NSString *key in [modelClass propertyKeys]) {
		...
		if (*(attributes->type) == *(@encode(id))) {
			...
			if (transformer == nil) transformer = [NSValueTransformer mtl_validatingTransformerForClass:propertyClass ?: NSObject.class propertyName:key propertyFromClass:modelClass];
		} else {
			transformer = [self transformerForModelPropertiesOfObjCType:attributes->type] ?: [NSValueTransformer mtl_validatingTransformerForClass:NSValue.class propertyName:key propertyFromClass:modelClass];
		}
...
//			if (JSONDictionary == nil) return nil;
            if (JSONDictionary == nil || ((NSDictionary *)JSONDictionary).count==0) {
                return nil;
            }
