...
+ (NSValueTransformer<MTLTransformerErrorHandling> *)mtl_validatingTransformerForClass:(Class)modelClass propertyName:(NSString *)propertyName propertyFromClass:(Class)propertyClass {
    ...
    return [MTLValueTransformer transformerUsingForwardBlock:^ id (id value, BOOL *success, NSError **error) {
        if (value != nil && ![value isKindOfClass:modelClass]) {
            ...
            }else if ([modelClass isSubclassOfClass:[NSNumber class]] && [value isSubclassOfClass:[NSString class]]){
                return [NSNumber numberWithInt:(int)[(NSNumber *)value integerValue]];
            }
        #endif
                NSDictionary *userInfo = @{
                                           NSLocalizedDescriptionKey: NSLocalizedString(@"Value did not match expected type", @""),
                                           NSLocalizedFailureReasonErrorKey: [NSString stringWithFormat:NSLocalizedString(@"Expected %1$@ to be of class %2$@ but got %3$@ to file %4$@ , propertyName:%5$@", @""), value, modelClass, [value class],propertyClass,propertyName],
